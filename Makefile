...
SOURCE_FILE = src/docker-health.cr
OUTPUT_DIR  = bin
OUTPUT_FILE = docker-health
...
install: ## Install docker-health in $(INSTALL_DIR)
	$(sudo) cp $(OUTPUT_DIR)/$(OUTPUT_FILE) $(INSTALL_DIR)/docker-health

uninstall: ## Uninstall docker-health from $(INSTALL_DIR)
	$(sudo) rm -f $(INSTALL_DIR)/docker-health

release-static: ## Build static binary with Earthly
	docker buildx bake binary
	mv packages/linux_arm64/docker-health-linux-arm64 packages/
	mv packages/linux_amd64/docker-health-linux-amd64 packages/
